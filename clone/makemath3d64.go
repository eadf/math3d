package main
/*
This is a quick and dirty hack to copy the code from the float32 package (math3d32) into the float64 package (math3d64)
*/
import "os"
import "bufio"
import "fmt"
...
func findandreplace(line string) string {
	...
	line = strings.Replace(line,"package math3d32","package math3d64\n//This code is auto generated from the math3d32 package. Do not edit.\nimport \"math\"",-1)
	line = strings.Replace(line,"Vector4f","Vector4d",-1)
	line = strings.Replace(line,"math3d/math3d32","math3d/math3d64",-1)
...
func main() {
	...
	for i:=0;i<len(files);i++{
		e := convertFile(fmt.Sprintf("math3d32/%s",files[i]),fmt.Sprintf("math3d64/%s",files[i]))
		if e != nil {
			fmt.Println(e)
			break
		}
